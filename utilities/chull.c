/*
  Update by Zebin Xu on 12/17/2017 to include read/write functions for obj file.
  You can run this program to generate the 3d convex hull of the model
  and save them into a file named ch3d.obj.

  Input: 3n integer coordinates for the points.
  Output: the 3D convex hull, stored in "ch3d.obj".
  Usage:
  ./chull verts.obj
*/
...
void    ReadRandomVertices(int num);
void    Print( void );
...
int main( int argc, char *argv[] )
{
  ...
  // }
  if (argc == 1) {
      ReadVerticesFromFile("verts.obj");
  } else if (argc == 2) {
      ReadVerticesFromFile(argv[1]);
      //    ReadRandomVertices(atoi(argv[1]));
  }
   // ReadVertices();
   DoubleTriangle();
   ConstructHull();
...
void    ReadRandomVertices(int num){
  printf("reading %d random vertices...\n", num);
  tVertex  v;
  int x, y, z;
  int vnum = 0;
...
void	ReadVerticesFromFile( const char* filename )
{
   printf("reading vertices from file...\n");
   tVertex  v;
   int      x, y, z;
   int	    vnum = 0;
   ...
   int verts[10000];//I know this is bad :(
   int indices[10000];
   int vcount = 0;
   int icount = 0;
   while ((read = getline(&line, &len, stream)) != -1) {
       char * pch;
       pch = strtok(line, " ");
       if (strcmp(pch, "v") == 0) {       //vertices
           while (pch != NULL) {
               pch = strtok(NULL, " \n");
               if (pch != NULL) {
                   verts[vcount++] = atoi(pch);
               }
           }
       } else if (strcmp(pch, "f") == 0) {       //indices
           while (pch != NULL) {
               pch = strtok(NULL, " \n");
               if (pch != NULL) {
                   indices[icount++] = atoi(pch);
               }
           }
       }
       printf("%d, %d, %d\n", verts[vcount-3], verts[vcount-2], verts[vcount-1]);
   }
   ...
   for (i = 0; i < icount; i +=1 ) {
      v = MakeNullVertex();
      v->v[X] = verts[indices[i]*3];
      v->v[Y] = verts[indices[i]*3+1];
      v->v[Z] = verts[indices[i]*3+2];
      v->vnum = vnum++;
      if ( ( abs(x) > SAFE ) || ( abs(y) > SAFE ) || ( abs(z) > SAFE ) ) {
         printf("Coordinate of vertex below might be too large: run with -d flag\n");
         PrintPoint(v);
...
void WriteObjFile(void) {
   ...
   //get the max indices to create mappings(table) index -> X, Y, Z
   int maxIndices = 0;
   do {                                 
      maxIndices = v->vnum > maxIndices ? v->vnum : maxIndices;
      v = v->next;
   } while ( v != vertices );
   printf("max indices: %d\n", maxIndices);

   //mappings: index -> X, Y, Z
   int vertTable[maxIndices+1][3];
   // set empty first as a flag
   for (int i = 0; i < maxIndices+1; i++) {
      vertTable[i][0] = EMPTY; vertTable[i][1] = EMPTY; vertTable[i][2] = EMPTY;
   }

   printf("%%%% index:\tx\ty\tz\n");
   do {                                 
      printf( "%%%% %5d:\t%d\t%d\t%d\n", v->vnum, v->v[X], v->v[Y], v->v[Z] );
      vertTable[v->vnum][0] = v->v[X];
      vertTable[v->vnum][1] = v->v[Y];
      vertTable[v->vnum][2] = v->v[Z];
      v = v->next;
   } while ( v != vertices );

   for (int i = 0; i <= maxIndices; i++) {
      if (vertTable[i][0] == EMPTY && vertTable[i][1] && EMPTY && vertTable[i][2] == EMPTY) continue; //skip
      printf("%d: %d %d %d\n", i, vertTable[i][0], vertTable[i][1], vertTable[i][2]);
   }
   ...
   // set empty first as a flag
   for (int i = 0; i < F; i++) {
      faceTable[i][0] = EMPTY; faceTable[i][1] = EMPTY; faceTable[i][2] = EMPTY;
   } 

   printf("%%%% Visible faces only: \n");
   int visibleFaceNum = 0;
   do {           
      /* Print face only if it is visible: if normal vector >= 0 */
      SubVec( f->vertex[1]->v, f->vertex[0]->v, a );
      SubVec( f->vertex[2]->v, f->vertex[1]->v, b );	  
      // if(( a[0] * b[1] - a[1] * b[0] ) >= 0 ) {
      //    //store visible faces only
      //    faceTable[fcount][0] = f->vertex[0]->vnum;
      //    faceTable[fcount][1] = f->vertex[1]->vnum;
      //    faceTable[fcount++][2] = f->vertex[2]->vnum;
      //    visibleFaceNum++;
      // }
   ...
   for (int i = 0; i < visibleFaceNum; i++) {
      printf("%d: %d %d %d\n", i, faceTable[i][0], faceTable[i][1], faceTable[i][2]);
   }
   int oldNewIndex[maxIndices+1];
   for (int i = 0; i < maxIndices+1; i++) {
      oldNewIndex[i] = EMPTY;
   } 
   int oncount = 0;
   //reorder the indices to start from 1, 2, 3, ...
   for (int i = 0; i <= maxIndices; i++) {
      if (vertTable[i][0] == EMPTY && vertTable[i][1] && EMPTY && vertTable[i][2] == EMPTY) continue; //skip
      oldNewIndex[i] = ++oncount;
   }
   for (int i = 0; i <= maxIndices; i++) {
      if (oldNewIndex[i] == EMPTY) continue; //skip
      printf("%d -> %d\n", i, oldNewIndex[i]);
   }
   ...
   for (int i = 0; i <= maxIndices; i++) {
      if (vertTable[i][0] == EMPTY && vertTable[i][1] && EMPTY && vertTable[i][2] == EMPTY) continue; //skip
      printf("%d: %d %d %d\n", oldNewIndex[i], vertTable[i][0], vertTable[i][1], vertTable[i][2]);
   }
   for (int i = 0; i < visibleFaceNum; i++) {
      printf("%d %d %d\n", oldNewIndex[faceTable[i][0]], oldNewIndex[faceTable[i][1]], oldNewIndex[faceTable[i][2]]);
   }

   printf("writing to file\n");
   FILE *file = fopen("ch3d.obj", "w");
   if (file == NULL) {
   ...
   for (int i = 0; i <= maxIndices; i++) {
      if (vertTable[i][0] == EMPTY && vertTable[i][1] && EMPTY && vertTable[i][2] == EMPTY) continue; //skip
      fprintf(file, "v %d %d %d\n", vertTable[i][0], vertTable[i][1], vertTable[i][2]);
